...
import "./InteractiveMapZoomFlip.scss";
import * as d3 from "d3";
import { getMapData } from "../../actions/mapAction";
import dataD from "../../data/Wise_coord_2020.json";

export class InteractiveMapZoomFlip extends Component<any, any> {
    ...
    function f() {
      if (!(d3 && d3.event && d3.event.transform) || lastZoom !== d3.event.transform.k) {
        ...
        dataD.forEach((item: any) => {
          let Latitude = 0;
          if (item["Latitude"] > 0) {
            Latitude = -Math.abs(item["Latitude"]);
          } else {
            Latitude = Math.abs(item["Latitude"]);
          }
...
export default connect(mapStateToProps, mapDispatchToProps)(InteractiveMapZoomFlip);
